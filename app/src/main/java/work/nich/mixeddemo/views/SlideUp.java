...
public class SlideUp implements View.OnTouchListener, ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
    ...
    private int autoSlideDuration = 300; // 动画的时间
    ...
    public SlideUp(final View view) {
        ...
        view.setPivotY(0); // 不知道干嘛的 TODO 你再看看
        createAnimation();
        view.getViewTreeObserver() // 这里添加一个layout的监听，避免再layout的时候又回来了
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (touchableTop < touchedArea) { // 在动画进行的过程中，若所处位置大于限定的值的话使ACTION_MOVE的setTranslationY无效
                    canSlide = false;
                }
                break;
